...
-(void)defaultConfig {
    ...
    if (!self.maxLimitDate) {
        self.maxLimitDate = [NSDate date:@"2037-12-31 23:59" WithFormat:@"yyyy-MM-dd HH:mm"];
    }
    //最小限制
    if (!self.minLimitDate) {
        self.minLimitDate = [NSDate date:@"1997-01-01 00:00" WithFormat:@"yyyy-MM-dd HH:mm"];
    }
}
...
-(void)addLabelWithName:(NSArray *)nameArr {
    ...
    for (int i=0; i<nameArr.count; i++) {
        ...
        if (i == 0) {
            labelX = kPickerSize.width/(nameArr.count*2)+25+kPickerSize.width/nameArr.count*i;
        }
