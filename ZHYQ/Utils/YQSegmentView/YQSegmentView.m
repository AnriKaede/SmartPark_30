...
@interface YQPageTitleButton : UIButton
@end
@implementation YQPageTitleButton
...
@property (nonatomic, strong) YQSegmentConfigure *configure;
/// scrollView
...
@property (nonatomic, strong) UIImageView *narrowView;
...
//    [self addSubview:self.bottomSeparator];
    ...
    _narrowView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"indoorWifi_d_narrow"]];
    _narrowView.frame = CGRectMake((_indicatorView.frame.size.width - 11)/2, _indicatorView.frame.size.height - 10, 11, 6);
    [_indicatorView addSubview:_narrowView];
}
...
- (UIView *)indicatorView {
    if (!_indicatorView) {
        ...
        if (self.configure.indicatorStyle == SGIndicatorStyleDefault) {
            ...
            _indicatorView.y = self.height - indicatorViewH;
        } else {
...
- (void)setupTitleButtons {
    ...
    if (self.allBtnWidth <= self.bounds.size.width) { /// SGPageTitleView 不可滚动
        ...
        for (NSInteger index = 0; index < titleCount; index++) {
            YQPageTitleButton *btn = [[YQPageTitleButton alloc] init];
...
- (void)P_smallIndicatorScrollStyleDefaultWithProgress:(CGFloat)progress originalBtn:(UIButton *)originalBtn targetBtn:(UIButton *)targetBtn {
    ...
    CGFloat targetBtnX = CGRectGetMaxX(targetBtn.frame) - [self SG_widthWithString:targetBtn.currentTitle font:self.configure.titleFont] - 0.5 * (self.width / self.titleArr.count - [self SG_widthWithString:targetBtn.currentTitle font:self.configure.titleFont] + self.configure.indicatorAdditionalWidth);
    CGFloat originalBtnX = CGRectGetMaxX(originalBtn.frame) - [self SG_widthWithString:originalBtn.currentTitle font:self.configure.titleFont] - 0.5 * (self.width / self.titleArr.count - [self SG_widthWithString:originalBtn.currentTitle font:self.configure.titleFont] + self.configure.indicatorAdditionalWidth);
    CGFloat totalOffsetX = targetBtnX - originalBtnX;
    
    /// 计算 targetBtn／originalBtn 宽度的差值
    CGFloat targetBtnDistance = (CGRectGetMaxX(targetBtn.frame) - 0.5 * (self.width / self.titleArr.count - [self SG_widthWithString:targetBtn.currentTitle font:self.configure.titleFont]));
    CGFloat originalBtnDistance = (CGRectGetMaxX(originalBtn.frame) - 0.5 * (self.width / self.titleArr.count - [self SG_widthWithString:originalBtn.currentTitle font:self.configure.titleFont]));
