...
- (void)_initView {
    ...
    stateLabel.frame = CGRectMake(18, 20, 70, 17);
    stateLabel.text = @"消息状态";
    stateLabel.textColor = [UIColor blackColor];
    [_bgView addSubview:stateLabel];
    
    UIButton *unReadBt = [UIButton buttonWithType:UIButtonTypeCustom];
    unReadBt.tag = 3000;
    unReadBt.frame = CGRectMake(stateLabel.left, stateLabel.bottom + 20, 100, 30);
    unReadBt.titleLabel.font = [UIFont systemFontOfSize:17];
    unReadBt.backgroundColor = [UIColor colorWithHexString:@"c7c7cd"];
    [unReadBt setTitle:@"未读" forState:UIControlStateNormal];
    [unReadBt setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [unReadBt setTitleColor:[UIColor whiteColor] forState:UIControlStateSelected];
    [unReadBt addTarget:self action:@selector(selUnRead) forControlEvents:UIControlEventTouchUpInside];
    [_bgView addSubview:unReadBt];
    
    UIButton *readBt = [UIButton buttonWithType:UIButtonTypeCustom];
    readBt.tag = 3001;
    readBt.frame = CGRectMake(unReadBt.right + 30, unReadBt.top, 100, 30);
    readBt.titleLabel.font = [UIFont systemFontOfSize:17];
    readBt.backgroundColor = [UIColor colorWithHexString:@"c7c7cd"];
    [readBt setTitle:@"已读" forState:UIControlStateNormal];
    [readBt setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [readBt setTitleColor:[UIColor whiteColor] forState:UIControlStateSelected];
    [readBt addTarget:self action:@selector(selRead) forControlEvents:UIControlEventTouchUpInside];
    [_bgView addSubview:readBt];
    
    UIView *topLineView = [[UIView alloc] initWithFrame:CGRectMake(0, readBt.bottom + 15, KScreenWidth, 1)];
...
// 选中未读
- (void)selUnRead {
    UIButton *unReadBt = [_bgView viewWithTag:3000];
    UIButton *readBt = [_bgView viewWithTag:3001];
    
    unReadBt.selected = YES;
    unReadBt.backgroundColor = CNavBgColor;
    readBt.selected = NO;
    readBt.backgroundColor = [UIColor colorWithHexString:@"c7c7cd"];
}
// 选中未读
- (void)selRead {
    ...
    unReadBt.selected = NO;
    unReadBt.backgroundColor = [UIColor colorWithHexString:@"c7c7cd"];
    readBt.selected = YES;
    readBt.backgroundColor = CNavBgColor;
}
...
- (void)certainAction {
    ...
    NSString *state;
    if(unReadBt.selected){
        state = @"0";
    }else if(readBt.selected){
        state = @"1";
    }
    
    // 筛选
    if(_filterTimeDelegate && [_filterTimeDelegate respondsToSelector:@selector(filterWithStart:withEndTime:withState:)]){
        [_filterTimeDelegate filterWithStart:inputStartStr withEndTime:inputEndStr withState:state];
    }
}
