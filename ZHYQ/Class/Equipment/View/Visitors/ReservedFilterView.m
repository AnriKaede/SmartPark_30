...
@implementation ReservedFilterView
{
    UIView *_bgView;
...
- (void)_initView {
    ...
    operateKeyLabel.frame = CGRectMake(operateLabel.left, operateLabel.bottom + 40, operateLabel.width, 17);
    operateKeyLabel.text = @"车辆信息";
...
- (void)closeAction {
    if(_reservedFilterDelegate && [_reservedFilterDelegate respondsToSelector:@selector(reservedCloseFilter)]){
        [_reservedFilterDelegate reservedCloseFilter];
    }
}
...
- (void)resetAction {
    [self endEditing:YES];
    // 重置
    if(_reservedFilterDelegate && [_reservedFilterDelegate respondsToSelector:@selector(reservedResetFilter)]){
        [_reservedFilterDelegate reservedResetFilter];
    }
}
...
- (void)certainAction {
    ...
    // 筛选
    if(_reservedFilterDelegate && [_reservedFilterDelegate respondsToSelector:@selector(filterWithStart:withEndTime:withOperateMan:withOperateKey:)]){
        [_reservedFilterDelegate filterWithStart:inputStartStr withEndTime:inputEndStr withOperateMan:manTF.text withOperateKey:keyTF.text];
    }
}
