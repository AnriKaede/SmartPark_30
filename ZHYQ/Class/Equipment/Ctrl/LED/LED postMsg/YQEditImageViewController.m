...
#define IMAGE_HEIHT KScreenWidth * image.size.height/image.size.width

@interface YQEditImageViewController ()<UIScrollViewDelegate>

/* scrollView*/
@property(nonatomic,strong) YQImageScrollView * scrollView;
...
- (void)createUI{
    ...
    _scrollView = [[YQImageScrollView alloc]init];
    _scrollView.delegate = self;
    _scrollView.contentSize = CGSizeMake((KScreenWidth + 20) * (_images.count - 1), KScreenHeight - 64);
    _scrollView.contentOffset = CGPointMake((_currentOffset - 1) * (KScreenWidth + 20), 0);
...
- (void)createImageForScrollView{
    ...
    for (int i = 0; i < _images.count-1; i ++) {
        ...
        imageView.frame = CGRectMake((KScreenWidth + 20) * i, (_scrollView.frame.size.height - IMAGE_HEIHT)/2, KScreenWidth, IMAGE_HEIHT);
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    _currentOffset = scrollView.contentOffset.x/KScreenWidth + 1;
    self.title = [NSString stringWithFormat:@"%d/%d",_currentOffset ,(int)_images.count - 1];
}
