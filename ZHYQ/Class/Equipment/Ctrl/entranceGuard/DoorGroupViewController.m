...
@interface DoorGroupViewController ()<UITableViewDelegate, UITableViewDataSource>
{
...
@implementation DoorGroupViewController
...
- (void)viewDidLoad {
    ...
    // 添加数据
    TopMenuModel *model = [[TopMenuModel alloc] init];
    model.BUILDING_NAME = @"研发楼";
    model.BUILDING_ID = @"-11";
    model.IS_OUT = @"0";
    
    TopMenuModel *model2 = [[TopMenuModel alloc] init];
    model2.BUILDING_NAME = @"2号楼";
    model2.BUILDING_ID = @"-55";
    model2.IS_OUT = @"0";
    
    [_floorData addObject:model];
    [_floorData addObject:model2];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(topModel.IS_OUT.integerValue == 1){
        ...
        // 室内
        DoorMapViewController *doorMapVC = [[DoorMapViewController alloc] init];
        doorMapVC.floorModel = model;
        [self.navigationController pushViewController:doorMapVC animated:YES];
    }
}
