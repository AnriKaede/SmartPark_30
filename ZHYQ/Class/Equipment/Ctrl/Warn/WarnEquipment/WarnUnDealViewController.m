...
@interface WarnUnDealViewController ()<CYLTableViewPlaceHolderDelegate>
{
//    CGFloat _height;
...
@implementation WarnUnDealViewController
...
- (void)_loadData {
    ...
    [paramDic setObject:@"0" forKey:@"alarmState"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.wranBillStyle = WranUnDeal;
    cell.wranModel = _warnData[indexPath.row];
    return cell;
}
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if(_confirmWramWindowDelegate && [_confirmWramWindowDelegate respondsToSelector:@selector(showConfirm:)]){
        if(_warnData.count > indexPath.row){
            WranUndealModel *wranModel = _warnData[indexPath.row];
            [_confirmWramWindowDelegate showConfirm:wranModel];
        }
    }
}
