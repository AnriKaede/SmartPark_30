...
- (void)_initView {
    ...
    if(_parkConsumeModel.totalFee.floatValue > 0){
        _valueLabel.text = [NSString stringWithFormat:@"+%.2f 元", _parkConsumeModel.totalFee.floatValue/100];
    }else {
        _valueLabel.text = [NSString stringWithFormat:@"%.2f 元", _parkConsumeModel.totalFee.floatValue/100];
    }
    _carNoLabel.text = [NSString stringWithFormat:@"%@", _parkConsumeModel.carNo];
