...
/* Function: PLAY_SetEngine
   ...
   Parameter: nPort,channel no.
	     decodeType, Decode Engine£¨only for H264,hevc)
	     renderType, Render Engine
   Return: BOOL, successful return to TRUE, failed return to FALSE.   
   Example£ºif only set one Decode Engine£¬can set Render Engine With NOTSET,
			PLAY_SetEngine(0, DECODE_HW, RENDER_NOTSET);         */
/************************************************************************/
