...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    if (self.viewControllers.count > 0) {
        if ([viewController conformsToProtocol:@protocol(YQTransitionProtocol)] && [self isNeedTransition:viewController]) {
...
-(id<UIViewControllerAnimatedTransitioning>)navigationController:(UINavigationController *)navigationController animationControllerForOperation:(UINavigationControllerOperation)operation fromViewController:(UIViewController *)fromVC toViewController:(UIViewController *)toVC
{
    ...
    if ([fromVC conformsToProtocol:@protocol(YQTransitionProtocol)] && [toVC conformsToProtocol:@protocol(YQTransitionProtocol)]) {
        
        BOOL pinterestNedd = [self isNeedTransition:fromVC:toVC];
        YQTransition *transion = [YQTransition new];
        ...
        return transion;
    }else if([toVC conformsToProtocol:@protocol(YQTransitionProtocol)]){
...
-(BOOL)isNeedTransition:(UIViewController<YQTransitionProtocol> *)fromVC :(UIViewController<YQTransitionProtocol> *)toVC
{
    BOOL a = NO;
    BOOL b = NO;
...
//判断fromVC和toVC是否需要实现pinterest效果
-(BOOL)isNeedTransition:(UIViewController<YQTransitionProtocol> *)toVC
{
    BOOL b = NO;
    if ([toVC respondsToSelector:@selector(isNeedTransition)] && [toVC isNeedTransition]) {
        b = YES;
