...
-(void)addBadgeViews{
    ...
    CGFloat itemWidth = self.bounds.size.width/3;;
    CGFloat itemLeft = tabIconWidth/2;
    if(KScreenWidth > 414){
        // ipa
        itemWidth = self.bounds.size.width/3 / itemsCount;
        itemLeft += self.bounds.size.width/3 + 20;
    }else {
        itemWidth = self.bounds.size.width / itemsCount;
    }
    ...
    for(int i = 0;i < itemsCount;i ++){
        ...
        redDot.center = CGPointMake(itemWidth*(i+0.5)+itemLeft, badgeTop);
